...
def reconstruct(text,seed,policy):
    ...
    if policy=="shuffle_precomputed":
        for vals in adj.values(): rng.shuffle(vals)
    elif policy=="reverse":
        for vals in adj.values(): vals.reverse()
    elif policy=="sorted":
        for vals in adj.values(): vals.sort()
    stack=[(s[0],s[1])]; path=[]
    while stack:
        v=stack[-1]
        if adj[v]:
            if policy=="dynamic_choice": c=adj[v].pop(rng.randrange(len(adj[v])))
            else: c=adj[v].pop()
            stack.append((v[1],c))
        else: path.append(stack.pop())
    vertices=list(reversed(path)); out=vertices[0][0]+vertices[0][1]+"".join(v[1] for v in vertices[1:])
...
def run():
    ...
    for name,text in CORPUS.items():
        original=len(zlib.compress(text.encode('utf-8'),9)); result[name]={}
        for policy in ('shuffle_precomputed','dynamic_choice','reverse','sorted'):
            seqs=[reconstruct(text,SEED+i,policy) for i in range(N_RUNS)]
            vals=[len(zlib.compress(x.encode('utf-8'),9)) for x in seqs]
            result[name][policy]={'original_zlib':original,'mean_zlib':statistics.mean(vals),'sd_zlib':statistics.stdev(vals),'min_zlib':min(vals),'max_zlib':max(vals),'delta_mean':statistics.mean(vals)-original,'distinct_reconstructions':len(set(seqs))}
    return {'experiment':'Ω-INF-6','date':'2026-08-13','seed':SEED,'runs_per_policy':N_RUNS,'question':'Is the Ω-INF-5 variation dependent on the reconstruction sampling policy?','result':result,'interpretation_limit':'Sampling-method control only; no semantic or universal claim.'}

if __name__=='__main__':
    out=Path(__file__).with_name('RESULTS_Omega-INF-6.json'); out.write_text(json.dumps(run(),ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(json.dumps(run(),ensure_ascii=False,indent=2))
