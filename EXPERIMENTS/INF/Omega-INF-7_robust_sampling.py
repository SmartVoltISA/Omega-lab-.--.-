...
This file is intentionally a protocol + executable experiment. It does not
state a desired direction for compression.
"""
...
def run():
    ...
    for name, text in CORPUS.items():
        ...
        seen = set()
        for seed in SEEDS:
            for i in range(N_PER_CORPUS // len(SEEDS)):
                transformed = reconstruct(text, seed * 100000 + i)
                seen.add(transformed)
                size = len(zlib.compress(transformed.encode("utf-8"), 9))
                rows.append(size - original)
        result[name] = {
            "length": len(text),
            "original_zlib": original,
            "n_reconstructions": len(rows),
            "distinct_reconstructions": len(seen),
            "mean_delta_zlib": statistics.mean(rows),
            "median_delta_zlib": statistics.median(rows),
            "sd_delta_zlib": statistics.stdev(rows),
            "min_delta_zlib": min(rows),
            "max_delta_zlib": max(rows),
    ...
    return {
        "experiment": "Ω-INF-7",
        "date": "2026-08-13",
        "question": "Do trigram-preserving reconstruction effects remain stable across more texts and seeds?",
        "runs_per_corpus": N_PER_CORPUS,
        "seeds": SEEDS,
        "results": result,
        "interpretation_limit": "No semantic claim; compression is one observable. This experiment tests robustness of the sampling procedure, not a universal law of information."
    }
