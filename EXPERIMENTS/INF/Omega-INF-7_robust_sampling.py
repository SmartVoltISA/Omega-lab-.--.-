...
def reconstruct(text, seed):
    rng = random.Random(seed); adj = build_adjacency(text)
...
def run():
    ...
    for name, text in CORPUS.items():
        original = len(zlib.compress(text.encode("utf-8"), 9)); rows = []; seen = set()
...
if __name__ == "__main__":
    ...
    data = run(); output.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8'); print(json.dumps(data, ensure_ascii=False, indent=2))
